...
###### [连续子数组的最大和（重要）](https://leetcode-cn.com/problems/lian-xu-zi-shu-zu-de-zui-da-he-lcof/)
...
func maxProfit(prices []int) int {
     ...
     for i:=1;i<len(prices);i++{
         minP =  min(minP,prices[i]) // 到这一天的最低价格
         maxP = max(maxP,prices[i]-minP); // 当前天 减去以前的最低价格
     } 
    return maxP  
}
...
###### [礼物的最大值](https://leetcode-cn.com/problems/li-wu-de-zui-da-jie-zhi-lcof/)
...
###### [零钱兑换二](https://leetcode-cn.com/problems/coin-change-2/submissions/)
...
##### [组合问题](https://leetcode-cn.com/problems/combinations/)
...
func run(n,k,start int,item []int){
    if len(item) == k {
        tmp := make([]int,k)
        copy(tmp,item)
        res = append(res,tmp)
    ...
    for i:=start;i+(k-len(item))-1<=n;i++{
        run(n,k,i+1,append(item,i))
    }
}
```
...
- G是goroutine实现的核心结构，它包含了栈，指令指针，以及其他对调度goroutine很重要的信息，例如其阻塞的channel。
  ...
  https://xuchen.youtuc.cn/2020/04/11/Go/GMP%E6%A8%A1%E5%9E%8B/
...
​	   1.下载：  https://github.com/protocolbuffers/protobuf/

​           https://github.com/protocolbuffers/protobuf/releases/download/v3.18.0/protoc-3.18.0-osx-x86_64.zip

​		2.解压缩，将命令文件的位置，添加到环境变量

​		3.运行``` protoc```

​		4.安装go的插件:https://github.com/grpc-ecosystem/grpc-gateway
...
package coolcar;
option go_package = "coolcar/proto/gen;trippb"; // 生成文件的目录名和包名
...
--go_out // 产出的go文件
plugins=grpc // 
paths=source_relative:gen/go // 当前目录下的gen/go文件夹
trip.proto // 生成的文件名
```
